...
description: Tassonomia opzionale bec_unit_category — attiva nella schermata permalink, slug URL, sync da Kross, archivi pubblici ed etichette multilingua.
---
...
Le categorie si configurano nella stessa schermata dei permalink unità: **Booking Engine → Units — permalinks**.
...
|--------------|-------------|
| **Abilita la tassonomia Unit Category** | Attiva la tassonomia: UI in admin (barra laterale modifica unità, colonne elenco), URL pubblici opzionali, REST per i termini quando è attiva, e **assegnazione alla sincronizzazione** dopo ogni upsert unità. |
| **Slug URL categoria** | Segmento di percorso prima dello slug del termine (base predefinita **`unit-category`** se il campo è vuoto). Esempio: `https://example.com/unit-category/mia-categoria/`. |
...
- **[Permalink e archivio](./05-permalinks-and-archive.md)** — slug unità, archivio e la stessa schermata admin.
